...
class Lab(models.Model):
    ...
    #     Line1 = 1
    #     Line2 = 2
    line = models.ForeignKey('general.Line',on_delete=models.CASCADE, verbose_name='خط')   # https://docs.djangoproject.com/en/3.0/ref
    # /models/fields/#field-choices
    # MINE_CHOICES = [('MIX', 'Mix'), ('CHESHMEH', 'CheshmehParvar'), ('GORGINEH', 'Gorgineh'), ]
    # mine = models.CharField(max_length=10, choices=MINE_CHOICES, default='MIX', verbose_name='معدن')
    mine = models.ForeignKey('general.Quarry', on_delete=models.CASCADE, verbose_name='معدن')
    # HOUR_CHOICES = [(dt.time(hour=x), '{:02d}:00'.format(x)) for x in range(0, 24)]
    # hour = models.TimeField(choices=HOUR_CHOICES, verbose_name='ساعت آزمایش')
    ...
    einstrumenge = models.PositiveSmallIntegerField(verbose_name='ملات',
                                               validators=[MinValueValidator(120, 'enter a number in range [120-160]'),
                                                           MaxValueValidator(160, 'enter a number in range [120-160]')],
                                               )
    # PRODUCT_CHOICES = {('K1', 'سیلو کناف 1'), ('K2', 'سیلو کناف 2'), ('B1', 'سیلو بلوک 1'), ('B2', 'سیلو بلوک 2'),
    #                    ('NP', 'بدون تولید'), ('800', 'سیلو 800 تنی'), }
    product_silo = models.ForeignKey('general.Silo',on_delete=models.CASCADE, verbose_name='محصول به سیلوی')
    # class Meta:
    #     ordering = ['-PRODUCT_CHOICES']
    free_moisture = models.DecimalField(max_digits=4, decimal_places=2, blank=True, null=True, verbose_name='رطوبت آزاد')
    crystal_water_rawmat = models.DecimalField(max_digits=5, decimal_places=2, blank=True, null=True, verbose_name='آب کریستال مواد خام',
    ...
    retained_63_micron = models.DecimalField(max_digits=4, decimal_places=2, blank=True, null=True, verbose_name='الک 63 میکرون',
                                             validators=[MinValueValidator(20.0, 'enter a number in range [20-40]'),
                                                         MaxValueValidator(40.0, 'enter a number in range [20-40]')],
                                             )
    retained_200_micron = models.DecimalField(max_digits=4, decimal_places=2, blank=True, null=True, verbose_name='الک 200 میکرون',
                                              validators=[MinValueValidator( 2.0, 'enter a number in range [2-20]'),
                                                          MaxValueValidator( 20.0, 'enter a number in range [2-20]')],
                                              )
    retained_500_micron = models.DecimalField(max_digits=4, decimal_places=2, blank=True, null=True, verbose_name='الک 500 میکرون',
                                              validators=[MinValueValidator( 0.0, 'enter a number in range [0-9]'),
                                                          MaxValueValidator( 9.0, 'enter a number in range [0-9]')],
                                              )
    soluble_anhydrite = models.DecimalField(max_digits=4, decimal_places=2, blank=True, null=True, verbose_name='انیدریت محلول')
    hemihydrate = models.DecimalField(max_digits=4, decimal_places=2, blank=True, null=True, verbose_name='همی هیدرات')
    low_solubility_anhydrite = models.DecimalField(max_digits=4, decimal_places=2, blank=True, null=True, verbose_name='انیدریت کم محلول')
    non_conformity = models.BooleanField(default=False, verbose_name='عدم انطباق')
    nonconf_P6 = models.BooleanField(default=False, verbose_name='عدم انطباق الک 63 میکرون', editable=When(non_conformity=True))
    nonconf_P5 = models.BooleanField(default=False, verbose_name='عدم انطباق الک 500 میکرون', editable=When(non_conformity=True))
    nonconf_P2 = models.BooleanField(default=False, verbose_name='عدم انطباق الک 200 میکرون', editable=When(non_conformity=True))
    nonconf_IS = models.BooleanField(default=False, verbose_name='عدم انطباق گیرش اولیه', editable=When(non_conformity=True))
    nonconf_FS = models.BooleanField(default=False, verbose_name='عدم انطباق گیرش ثانویه', editable=When(non_conformity=True))
    nonconf_EI = models.BooleanField(default=False, verbose_name='عدم انطباق وزن ملات', editable=When(non_conformity=True))
    nonconf_FM = models.BooleanField(default=False, verbose_name='عدم انطباق رطوبت آزاد', editable=When(non_conformity=True))
    nonconf_CS = models.BooleanField(default=False, verbose_name='عدم انطباق آب کریستال گچ', editable=When(non_conformity=True))
    # ACTION_CHOICES = {('CP', 'اجازه ارفاقی'), ('DP', 'اجازه انحراف'), ('SC', 'اسقاط'), ('RG', 'درجه بندی مجدد')}
    burner_sp_change = models.DecimalField(blank=True, null=True, verbose_name='تغییر دمای مشعل', max_digits=3, decimal_places=1, )
    call_to_operator = models.ForeignKey('auth.User', on_delete=models.CASCADE, related_name='call_to_operator', null=True, blank=True, verbose_name='تماس با اپراتور')
    # action = models.CharField(max_length=5, choices=ACTION_CHOICES, blank=True, verbose_name='تعیین تکلیف')
    action = models.ForeignKey('general.Action',on_delete=models.CASCADE, blank=True, null=True, verbose_name='تعیین تکلیف')
...
# class LabRetest(models.Model):
#
#     hour = models.ForeignKey(Lab,on_delete=models.CASCADE)
#     quarter = models.ForeignKey(Quarter, on_delete=models.SET_NULL, null=True)
#     einstrumenge = models.PositiveSmallIntegerField( verbose_name='ملات',
#                                                  validators=[
